...
{
    [ProviderInfo(Name = "Podio Contacts", Description = "Read/Write contacts from Podio", Group = "Podio")]
    [ProviderIgnore]
    public class PodioContactsDataSourceReader : DataReaderProviderBase, IDataSourceRegistry
    {
        ...
        public PodioContactsDataSourceReader()
        {
        ...
        public override DataTableStore GetDataTable(DataTableStore dt)
        {
            ...
            do
            {
                HttpWebRequest webRequest = WebRequest.CreateHttp(string.Format("https://api.podio.com/contact/space/{0}/?contact_type=space&exclude_self=false&type=mini&limit={1}&offset={2}", SpaceID, Limit, offset));
                ...
                using (HttpWebResponse response = (HttpWebResponse)webRequest.GetResponse())
                {
                    ...
                    using (StreamReader sr = new StreamReader(response.GetResponseStream()))
                    {
                        ...
                        foreach (var item_row in result)
                        {
                            ...
                            if (dt.Rows.AddWithIdentifier(newRow, PodioDataSchemaTypeConverter.ConvertToInvariant<long>(item_row["profile_id"])) == DataTableStore.ABORT)
                            {
                                abort = true;
                                return dt;
                            }
        ...
        {
            PodioSchema = PodioDataSchema.Contacts();
            return PodioSchema.ToDataSchema();
        }

        private void UpdateRateLimits(HttpWebResponse response)
        {
            int val;
        ...
        private bool ValidateToken()
        {
            ...
            if (TokenExpires < DateTime.Now.AddHours(-1))
            {
        ...
        {
            return new PodioContactsDataSourceWriter { SchemaMap = SchemaMap };
        }
        ...
        internal dynamic GetContact(long profile_id)
        {
            ValidateToken();

            HttpWebRequest webRequest = WebRequest.CreateHttp(string.Format("https://api.podio.com/contact/{0}/v2", profile_id));
            webRequest.UserAgent = PodioHelper.USER_AGENT;
            webRequest.Method = "GET";
            webRequest.ServicePoint.Expect100Continue = false;
            webRequest.Headers.Add("Authorization", string.Format("OAuth2 {0}", AccessToken));
        ...
        public object GetRegistryInterface()
        {
            ...
            {
                return new PodioContactsDataSourceReaderWithRegistry(this);
            }
        }
    ...
    public class PodioContactsDataSourceReaderWithRegistry : DataReaderRegistryView<PodioContactsDataSourceReader>
    {
        [Category("Connection.Library")]
        [Description("Key Name of the Item in the Connection Library")]
        [DisplayName("Key")]
        ...
        [Category("Podio Api")]
        [ReadOnly(true)]
        public int ApiRateLimit { get { return _reader.ApiRateLimit; } set { _reader.ApiRateLimit = value; } }

        [Category("Podio Api")]
        [ReadOnly(true)]
        public int ApiRateRemaining { get { return _reader.ApiRateRemaining; } set { _reader.ApiRateRemaining = value; } }
        ...
        public PodioContactsDataSourceReaderWithRegistry(PodioContactsDataSourceReader reader)
            : base(reader)
        {
